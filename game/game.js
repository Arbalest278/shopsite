...
fonimg.src = "./files/background.jpg";
...
function update() {
    ...
    for (i in aster) {
        ...
        for (j in fire) {
            if (Math.abs(aster[i].x - fire[j].x - 15) < 70
...
function render() {
    context.drawImage(fonimg, 0, 0, 1600, 600);
    context.drawImage(shipimg, ship.x, ship.y, 60, 60);
    for (i in aster) {
        context.drawImage(asterimg, aster[i].x, aster[i].y, 70, 70)
    }
    for (i in fire) {
